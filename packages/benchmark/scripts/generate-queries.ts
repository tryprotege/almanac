...
const main = async () => {
  const dataPath = path.join(__dirname, '../../benchmarking/output/combined/grouped.json');
  ...
  console.log(`Found ${groups.length} workflow groups, randomly selected ${selectedGroups.length}`);
  ...
  for (let index = 0; index < selectedGroups.length; index++) {
    ...
    try {
      ...
      console.error(`❌ Failed to generate queries for group ${index + 1}:`, err);
    }
  }
  ...
  console.log(
    `\n📊 Total test cases: ${allGroups.reduce((sum, g) => sum + g.testCases.length, 0)}`,
  );
};
