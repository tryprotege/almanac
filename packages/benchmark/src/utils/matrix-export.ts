...
async function exportDetailedTestResults(
  ...
  const resultsByQuery = new Map<
    string,
    Array<(typeof results.detailedResults)[number]>
  >();
  ...
  for (const result of results.detailedResults) {
    ...
    const evaluationScore = result.evaluation
      ? result.evaluation.score.toFixed(3)
      : "N/A";

    const matchRate = result.evaluation
      ? (
          (result.evaluation.matchedCount / result.evaluation.totalRequired) *
          100
        ).toFixed(1)
      : "N/A";

    const matchedCount = result.evaluation
      ? `${result.evaluation.matchedCount}`
      : "N/A";

    const totalRequired = result.evaluation
      ? `${result.evaluation.totalRequired}`
      : "N/A";
 ...
 */
function getExpectedOutputForQuery(
  results: MatrixBenchmarkResults,
  queryId: string
): string {
  // Try to find the scenario in the config
  if (
    results.config.queriesSource.type === "hardcoded" &&
  ...
  const resultWithEval = results.detailedResults.find(
    (r) => r.queryId === queryId && r.evaluation
  );
