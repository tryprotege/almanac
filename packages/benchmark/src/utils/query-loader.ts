...
export function loadScenarios(
  ...
  if (queriesSource.type === 'hardcoded') {
    if (!hardcodedScenarios || hardcodedScenarios.length === 0) {
      throw new Error('Hardcoded query source specified but no scenarios provided in config');
    }
  ...
  const filteredWorkflows = workflows.filter((w) => !skipWorkflows.has(w.workflow.groupId));
  ...
  for (const workflow of filteredWorkflows) {
    workflow.testCases.forEach((testCase, index) => {
      scenarios.push(testCaseToScenario(testCase, workflow.workflow.groupId, index));
    });
  }
