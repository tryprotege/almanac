...
function flattenMessages(messagesMap: Map<string, MessageElement[]>): MessageElement[] {
...
export function buildWorkContext(foundation: AllGeneratedData): Partial<RelationshipContext> {
  ...
  const workPages = foundation.notion.pages.filter((p) => p.id.includes('work'));
...
export function buildCasualContext(foundation: AllGeneratedData): Partial<RelationshipContext> {
  const allMessages = flattenMessages(foundation.slack.messages);
  ...
  const personalPages = foundation.notion.pages.filter((p) => p.id.includes('personal'));
