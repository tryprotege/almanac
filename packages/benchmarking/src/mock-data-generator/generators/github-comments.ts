  ...
  for (const issue of issues) {
    ...
    for (let i = 0; i < commentCount; i++) {
      ...
      if (i === 0) {
        ...
        commentType = selectRandom(['suggestion', 'question', 'context'] as const);
      }
...
${commentType === 'suggestion' ? '- Suggests a fix or approach to solve the issue' : ''}
${commentType === 'confirmation' ? '- Confirms the issue is resolved or provides closure' : ''}
${commentType === 'context' ? '- Provides additional context or reproduction steps' : ''}
...
Return ONLY the comment text (no JSON, no quotes):`;
      ...
      try {
        ...
        console.error(`Error generating comment ${i + 1} for issue #${issue.number}:`, error);
      }
    }
...
export async function generateGitHubPRReviews(
  ...
  console.log(`Generating reviews for ${pullRequests.length} GitHub pull requests...`);
  ...
  for (const pr of pullRequests) {
    ...
    for (let i = 0; i < reviewers.length; i++) {
      ...
      const prMerged = pr.merged_at ? new Date(pr.merged_at).getTime() : prCreated + 86400000;
      const timeRange = prMerged - prCreated;
      const reviewTime = new Date(prCreated + (timeRange * (i + 1)) / (reviewers.length + 1));
...
${
  !isApproval ? '- "Consider extracting this into a separate function for better testability."' : ''
}
...
Return ONLY the comment text (no JSON, no quotes):`;
      ...
      try {
        ...
        reviews.push({
          ...
          pull_request_url:
            pr.url || `https://api.github.com/repos/gragger/repo/pulls/${pr.number}`,
          submitted_at: reviewTime.toISOString(),
          commit_id: pr.merge_commit_sha || generateRandomHash(40),
          author_association: 'CONTRIBUTOR',
        });
      } catch (error) {
        console.error(`Error generating review ${i + 1} for PR #${pr.number}:`, error);
      }
    }
