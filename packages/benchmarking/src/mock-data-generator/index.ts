...
async function main() {
  ...
  console.log(`  Mode: ${dateRange.isInitialRun ? 'Initial Run' : 'Append Mode'}`);
  ...
  if (!dateRange.isInitialRun) {
    console.log(`  Dataset Start: ${existingMetadata!.startDate.split('T')[0]}`);
  ...
  if (!dateRange.isInitialRun) {
    ...
    if (existsSync(combinedPath)) {
      ...
      console.log(`  Loaded ${existingData.github.issues.length} GitHub issues`);
      console.log(`  Loaded ${existingData.github.pullRequests.length} GitHub PRs`);
      console.log(`  Loaded ${existingData.slack.messages.length} Slack messages`);
      console.log(`  Loaded ${existingData.notion.pages.length} Notion pages`);
      console.log(`  Loaded ${existingData.fathom.meetings.length} Fathom meetings`);
      console.log('='.repeat(50));
    }
  }

  // Stage 1: Foundation (40%)
  const foundation = await generateFoundation(config, volumes, dateRange.startDate, existingData);
  ...
  const combined = existingData
    ? {
        github: {
          ...
          pullRequests: [...newData.github.pullRequests, ...existingData.github.pullRequests],
        ...
        fathom: {
          meetings: [...newData.fathom.meetings, ...existingData.fathom.meetings],
          teams: existingData.fathom.teams,
          teamMembers: existingData.fathom.teamMembers,
          transcripts: [...newData.fathom.transcripts, ...existingData.fathom.transcripts],
          summaries: [...newData.fathom.summaries, ...existingData.fathom.summaries],
        },
      }
    : newData;

  writeFileSync(join(config.outputDir, 'combined', 'data.json'), JSON.stringify(combined, null, 2));
  ...
  console.log(`📅 Metadata saved with start date: ${metadata.startDate.split('T')[0]}`);
  console.log(
  ...
  console.log(`\n📦 Output (grouped.json):`);
  console.log(`  🔗 Workflow groups: ${grouped.metadata.summary.totalWorkflows}`);
  console.log(`  📊 Records in workflows: ${grouped.metadata.summary.totalRecordsInWorkflows}`);
  console.log(`  📝 Standalone items: ${grouped.metadata.summary.totalStandaloneRecords}`);
}
