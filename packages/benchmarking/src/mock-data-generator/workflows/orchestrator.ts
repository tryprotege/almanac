...
export class WorkflowOrchestrator {
  ...
  async generateConnectedData(
    workflowCount: number
  ): Promise<typeof this.artifacts> {
    ...
    for (let i = 0; i < workflowCount; i++) {
      ...
      console.log(
        `🔄 Generating workflow ${i + 1}/${workflowCount}: ${
          template.name
        } - "${topic.title}"`
      );
  ...
  private selectWorkflowTemplates(count: number): WorkflowTemplate[] {
    ...
    const totalWeight = WORKFLOW_TEMPLATES.reduce(
  ...
  private async executeWorkflow(
    ...
    for (const stage of template.stages) {
      ...
      const delayHours =
        ...
        Math.random() *
          (stage.delayFromPrevious.max - stage.delayFromPrevious.min);
      ...
      const references = stage.references
        .map((ref) => instance.artifacts.get(ref))
        .filter(Boolean);

      // Generate the artifact for this stage
      try {
        const artifact = await this.generateStageArtifact(
          stage,
          topic,
          currentDate,
          references
...
${
  references.some((r) => r?.type === "message")
    ? '- Mention "as discussed in Slack"'
    : ""
}
${
...
${
  references.some((r) => r?.recording_id)
    ? '- Reference "per our meeting discussion"'
    : ""
}
...
${
  references.some((r) => r?.recording_id)
    ? '- Should reference "the meeting" or "our call"'
    : ""
}
${
  references.some((r) => r?.url?.includes("notion"))
...
[
    ...
    return messages.map((msg: any, idx: number) => {
      const user =
        participants.find((p) =>
          p.real_name
            ?.toLowerCase()
            .includes(msg.user.toLowerCase().split(" ")[0])
  ...
  private async generateNotionArtifact(
    ...
    const prompt = `Generate a Notion ${pageTypeMap[type] || "Document"} for: ${
      topic.title
    }
...
${
  references.some((r) => r?.recording_id)
    ? '- Include "Meeting Reference" section'
    : ""
}
...
${
  type === "meeting-notes"
    ? "- Include Attendees, Discussion Points, Decisions, Action Items"
    : ""
}
${
  type === "spec-page"
    ? "- Include Overview, Requirements, Technical Approach, Open Questions"
    : ""
}
...
Return JSON:
{
  ...
  "content": "Full page content with markdown sections"
    ...
    return {
      ...
      url: `https://notion.so/${topic.title
        .replace(/\s+/g, "-")
        .toLowerCase()}`,
  ...
  private async generateFathomArtifact(
    ...
    const participants = topic.participants.map((name) => {
      ...
      return {
        ...
        email:
          member?.email ||
          `${name.toLowerCase().replace(" ", ".")}@gragger.com`,
    ...
    const prompt = `Generate a meeting transcript for: ${
      meetingTitleMap[type] || "Team Meeting"
    }
...
${
  type === "incident-call"
    ? "- Urgent tone, troubleshooting a production issue"
    : ""
}
...
Generate the transcript:`;
  ...
  private buildReferenceContext(references: any[]): string {
    ...
    for (const ref of references) {
      ...
      if (ref.thread_ts || ref.type === "message") {
        parts.push(
          `- Slack thread discussing: "${ref.text?.substring(0, 100)}..."`
        );
      }
      if (ref.url?.includes("notion")) {
        const title =
          ref.properties?.title?.title?.[0]?.plain_text || "Document";
  ...
  private selectChannelForTopic(topic: WorkflowTopic): Channel {
    ...
    return (
      this.slackChannels.find((c) => c.name === channelName) ||
      this.slackChannels[0]
    );
  }
