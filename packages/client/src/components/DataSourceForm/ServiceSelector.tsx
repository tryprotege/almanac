...
export function ServiceSelector({ onSelect }: ServiceSelectorProps) {
  ...
  return (
    <div className="p-6">
      <h3 className="text-lg font-semibold text-text-primary mb-2">Select MCP Service</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {presets.map((preset) => (
          <ServiceCard key={preset.id} preset={preset} onSelect={() => onSelect(preset)} />
        ))}
      </div>
    </div>
  );
    ...
    >
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-semibold text-text-primary mb-1">{preset.displayName}</h4>
          <p className="text-sm text-text-tertiary line-clamp-2">{preset.description}</p>
        </div>
      </div>
    </button>
  );
