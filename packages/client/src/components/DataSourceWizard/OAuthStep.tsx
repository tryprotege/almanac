...
export function OAuthStep({ serverConfig, onBack, onComplete, isLoading }: OAuthStepProps) {
  const [oauthStatus, setOauthStatus] = useState<'pending' | 'success' | 'error'>('pending');
    ...
    const fetchServerId = async () => {
      ...
      try {
        ...
        if (response.data.data?._id) {
          ...
          setErrorMessage('Server created but no ID returned. Please try refreshing the page.');
    ...
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-text-tertiary">
          This data source requires OAuth authorization to access your data securely.
        </p>
      </div>
      ...
      <div className="card">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <h3 className="text-lg font-medium text-text-primary mb-2">{serverConfig.name}</h3>

            {oauthStatus === 'pending' && (
              <div>
                <p className="text-text-secondary mb-4">
                  Click the button below to authorize {serverConfig.name} to access your data.
                  You'll be redirected to complete the authorization process.
                </p>
            ...
            {oauthStatus === 'success' && (
              <p className="text-brand-success">✓ Authorization successful! Proceeding...</p>
            )}

            {oauthStatus === 'error' && (
              <div>
                <p className="text-brand-error mb-2">Authorization failed: {errorMessage}</p>
