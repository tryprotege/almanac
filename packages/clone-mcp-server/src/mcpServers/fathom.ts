...
fathomMcpServer.registerTool(
  'list_meetings',
  {
    ...
    inputSchema: z.object({
      ...
      calendar_invitees_domains_type: z
        ...
        .optional()
        .describe('Filter by whether calendar invitee list includes external email domains'),
      created_after: z
        .string()
        .optional()
        .describe(
  ...
  async (args) => {
    ...
    if (args.cursor) {
      ...
      const cursorIndex = meetings.findIndex((m) => m.recording_id === cursorId);
      if (cursorIndex !== -1) {
...
fathomMcpServer.registerTool(
  ...
  async (args) => {
    ...
    const nextCursor = startIndex + limit < teams.length ? teams[startIndex + limit].id : null;
...
fathomMcpServer.registerTool(
  ...
  async (args) => {
    ...
    const nextCursor = startIndex + limit < members.length ? members[startIndex + limit].id : null;
