...
async function wipeMongoDB(keepMcpConfig: boolean, source?: string): Promise<void> {
  ...
  logger.info(`   ✓ Deleted ${embeddingResult.deletedCount} graph embedding metadata records`);
  ...
  if (!keepMcpConfig) {
    ...
    const dataSource = source ? await DataSourceModel.findOne(mcpFilter) : undefined;
    ...
    await OAuthTokenModel.deleteMany(dataSource ? { mcpServerConfigId: dataSource._id } : {});
  } else {
...
async function wipeMemgraph(
  ...
  try {
    if (source) {
      ...
      await memgraph.executeQuery(`MATCH (n {source: '${source}'})-[r]->() DELETE r`);
    ...
    if (!source) {
      ...
      try {
        await memgraph.executeQuery('DROP CONSTRAINT ON (n:Resource) ASSERT n.id IS UNIQUE');
    ...
    const { deleted, errors } = await cleanupOrphanedEmbeddings(graphStore, vectorStore);
    logger.info(
...
async function wipeQdrant(qdrant: any, source?: string): Promise<void> {
  ...
  try {
      ...
      if (source) {
        ...
        for (const collection of collections.collections) {
          try {
            ...
            deletedCount++;
            logger.info(`   ✓ Deleted vectors from collection: ${collection.name}`);
          } catch (err) {
            logger.warn(
    ...
    logger.info(`   ✓ Cleared timestamps for ${timestampResult.modifiedCount} records`);
  } catch (err) {
...
async function run() {
  ...
  try {
    ...
    logger.info("  3. Run 'pnpm tsx scripts/index-vectors.ts' to build embeddings");
  } catch (err) {
