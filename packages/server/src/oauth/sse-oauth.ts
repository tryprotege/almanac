 ...
 */
export async function discoverSseOAuth(
  sseUrl: string
): Promise<SseOAuthDiscoveryResult> {
  try {
    ...
    try {
      ...
      if (is401Error) {
        ...
        logger.info(
          { sseUrl, metadata },
          "Successfully discovered SSE OAuth metadata"
        );
