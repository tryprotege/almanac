    ...
    for (const [paramName, paramValue] of Object.entries(fetcher.params)) {
      ...
      if (valueStr.length > 50 && /[0-9a-f]{20,}/.test(valueStr)) {
        issues.push({
          ...
          suggestion: 'Verify this is a valid, dynamic parameter and not a hardcoded example.',
        });
      }
    }
  }
...
function validateEnrichmentPaths(
  ...
  for (const [recordTypeName, recordType] of Object.entries(config.recordTypes || {})) {
    ...
    for (const enrichment of recordType.enrichments) {
      ...
      for (const [paramName, paramPath] of Object.entries(enrichment.paramMapping)) {
 ...
 */
function validateDiscoveryMechanisms(config: IndexingConfig): PostValidationIssue[] {
  const issues: PostValidationIssue[] = [];
  ...
  for (const [recordTypeName, recordType] of Object.entries(config.recordTypes || {})) {
    ...
    if (fetcher.params && Object.keys(fetcher.params).length > 0) {
      ...
      const identifierParams = paramKeys.filter((k) => /^(id|url|key|slug|path)$/i.test(k));
...
function validateRecordIdFields(
  ...
  for (const [recordTypeName, recordType] of Object.entries(config.recordTypes || {})) {
    ...
    const hasCommonIdField = commonIdFields.some(
      (field) => sampleRecord[field] !== undefined && sampleRecord[field] !== null,
    );
 ...
 */
function validateEnrichmentOnlyTools(config: IndexingConfig): PostValidationIssue[] {
  const issues: PostValidationIssue[] = [];
  ...
  for (const [recordTypeName, recordType] of Object.entries(config.recordTypes || {})) {
    ...
    const isEnrichmentTool =
      ...
      (fetcher.params && Object.keys(fetcher.params).some((k) => k.endsWith('_id')));
...
function autoRepair(
    ...
    if (issue.path.endsWith('.arrayPath') && issue.severity === 'error') {
      ...
      if (match) {
        ...
        if (sample && fetcher && !fetcher.arrayPath) {
          ...
          if (arrayPath) {
            logger.info({ fetcherName, arrayPath }, 'Auto-repair: Adding missing arrayPath');
  ...
  for (const issue of issues) {
    if (issue.severity === 'error' && issue.message.includes('enrichment-only tool')) {
  ...
  for (const fetcherName of fetchersToRemove) {
    ...
    newConfig.syncOrder = newConfig.syncOrder.filter((name: string) => name !== fetcherName);
    repaired = true;
  }
...
function detectArrayPath(sample: any): string | null {
  ...
  const arrayFields = ['items', 'results', 'data', 'records', 'list', 'entries', 'content'];
  ...
  for (const wrapper of wrapperKeys) {
    if (sample[wrapper] && typeof sample[wrapper] === 'object') {
      for (const field of arrayFields) {
        if (Array.isArray(sample[wrapper][field]) && sample[wrapper][field].length > 0) {
