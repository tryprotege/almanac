...
class PresetLoaderService {
  ...
  async loadPresetsAtStartup(): Promise<void> {
    ...
    try {
      ...
      for (const file of jsonFiles) {
        try {
          ...
          if (!preset.id || !preset.displayName || !preset.connection) {
            console.warn(
              `[PresetLoader] Skipping invalid preset ${file}: missing required fields`
            );
            continue;
          }
          ...
          console.log(
            `[PresetLoader] ✓ Loaded preset: ${preset.displayName} (${preset.id})`
          );
        } catch (error) {
      ...
      this.loaded = true;
      console.log(
        `[PresetLoader] Successfully loaded ${this.presets.size} presets`
      );
    } catch (error) {
